...
class RequestGenerationTests(unittest.TestCase):
    def test_rcv2(self):
        ...
        request = requests.recaptcha_v2_proxyless_request(
            websiteUrl="some_url",
            websiteKey="sime_key",
            dataSValue="sdfa",
            userAgent="fasdf",
        ...
        proxy_request = requests.recaptcha_v2_request(
            websiteUrl="some_url",
            websiteKey="some_key",
        ...
        proxy_task = proxy_request.get_task_dict()
    ...
    def test_rcv3(self):
        ...
        request = requests.recaptcha_v3_proxyless_request(
            websiteUrl="some_url", websiteKey="some_key", min_score=0.2, pageAction="asdfsfd"
        )
        task = request.get_task_dict()

        for key in default_keys:
            self.assertIsNotNone(
                task.get(key), msg=f"Missing {key} for ReCaptchaV3Proxyless request."
            )
    ...
    def test_rcv2_enterprise(self):
        ...
        request = requests.recaptcha_v2_enterpise_proxyless_request(
            websiteUrl="some_url",
            websiteKey="some_key",
        ...
        for key in default_keys:
            self.assertIsNotNone(
                task.get(key), msg=f"Missing {key} for RecaptchaV2EnterpriseTaskProxyless request."
            )
        self.assertEqual(
            rcv2e_type,
            task.get("type"),
        ...
        for key in proxy_keys:
            self.assertIsNotNone(
                proxy_task.get(key), msg=f"Missing {key} for RecaptchaV2EnterpriseTask request."
            )
        self.assertEqual(
            proxy_type,
            proxy_task.get("type"),
    ...
    def test_fc(self):
        ...
        request = requests.funcaptcha_proxyless_request(
            websiteUrl="some_url",
        ...
        for key in default_keys:
            self.assertIsNotNone(
                task.get(key), msg=f"Missing {key} for FunCaptchaTaskProxyless request."
            )
        self.assertEqual(
            noproxy_type,
            task.get("type"),
        ...
        proxy_keys = default_keys + PROXY_LIST
        proxy_request = requests.funcaptcha_request(
            websiteUrl="some_url",
            websitePublicKey="some_key",
            funcaptchaApiJSSubdomain="domain",
            data="asdfasdf",
    ...
    def test_hc(self):
        ...
        request = requests.hcaptcha_proxyless_request(
            websiteUrl="some_url",
            websiteKey="some_key",
            is_invisible=False,
            data="data",
        ...
        for key in default_keys:
            self.assertIsNotNone(
                task.get(key), msg=f"Missing {key} for HCaptchaTaskProxyless request."
            )
        self.assertEqual(
            noproxy_type,
            task.get("type"),
        ...
        proxy_request = requests.hcaptcha_request(
            websiteUrl="some_url",
            websiteKey="some_key",
            is_invisible=False,
            data="data",
    ...
    def test_image2text(self):
        ...
        request = requests.image_to_text_request(
        ...
        task = request.get_task_dict()
        for key in default_keys:
            self.assertIsNotNone(task.get(key), msg=f"Missing {key} for ImageToTextTask request.")
        self.assertEqual(
            noproxy_type,
            task.get("type"),
    ...
    def test_gt(self):
        ...
        request = requests.geetest_roxyless_request(
            websiteUrl="some_url",
            gt="some_key",
            challenge="challenge",
        ...
        for key in default_keys:
            self.assertIsNotNone(
                task.get(key), msg=f"Missing {key} for GeeTestTaskProxyless request."
            )
        self.assertEqual(
            noproxy_type,
            task.get("type"),
        ...
        proxy_request = requests.geetest_request(
            websiteUrl="some_url",
            gt="some_key",
            challenge="challenge",
            geetestApiServerSubdomain="api.domain",
        ...
        proxy_keys = default_keys + PROXY_LIST
        for key in proxy_keys:
            self.assertIsNotNone(proxy_task.get(key), msg=f"Missing {key} for GeeTestTask request.")
        self.assertEqual(
            proxy_type,
            proxy_task.get("type"),
